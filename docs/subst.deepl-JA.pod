...
B<--dictdata>オプションは、コマンドラインで辞書データを提供するために使用することができます。
...
マッチした文字列が前にマッチした文字列と重なる場合、警告を出し (デフォルトでは B<--warn-overlap)、無視されます。
...
B<--diff>オプションで使用するdiffコマンド名を指定します。デフォルトは "diff
-u".
